...
get_metallo = (hass.states.get('input_text.metallo_hp').state).lower().replace(' ', '')
get_pannolini = (hass.states.get('input_text.pannolini_hp').state).lower().replace(' ', '')

attributi_input = {'min': 0, 'max': 255, 'mode': 'text', 'Package:': 'Raccolta Differenziata', 'Creato da:': 'Mattia(xxKira) e Fabio Bigmoby Mauro', 'Creato per:': 'HassioHelp'}
...
if get_metallo is None:
  hass.states.set('input_text.metallo_hp', '-', attributi_input )
else:
  get_metallo = get_metallo.split('-')
if get_pannolini is None:
  hass.states.set('input_text.pannolini_hp', '-', attributi_input )
else:
  get_pannolini = get_pannolini.split('-')
...
def set_rifiuti(hass, day, day_n, prima_settimana, get_vetro, get_verde, get_umido, get_plastica, get_secco, get_metallo, get_carta, get_pannolini):
  global attributi_sensor
  attributi_sensor = {'Package:': 'Raccolta Differenziata', 'Creato da:': 'Mattia(xxKira) e Fabio Bigmoby Mauro', 'Creato per:': 'HassioHelp'}
  ...
  secco = get_secco[0]
  metallo = get_metallo[0]
  pannolini = get_pannolini[0]
  carta = get_carta[0]
  vetro = vetro.split(",")
  verde = verde.split(",")
  umido = umido.split(",")
  ...
  secco = secco.split(",")
  metallo = metallo.split(",")
  carta = carta.split(",")
  pannolini = pannolini.split(",")
  ...
  try:
    ...
    secco_bis = []
  try:
    metallo_bis = get_metallo[1]
    metallo_bis = metallo_bis.split(',')
  except:
    metallo_bis = []    
  try:
    ...
    carta_bis = []
  try:
    pannolini_bis = get_pannolini[1]
    pannolini_bis = pannolini_bis.split(',')
  except:
    pannolini_bis = []    
  ...
  try:
    ...
    secco_bis_dis = []
  try:
    metallo_bis_dis = get_metallo[2]
    metallo_bis_dis = metallo_bis_dis.split(',')
  except:
    metallo_bis_dis = []    
  try:
    ...
    carta_bis_dis = []
  try:
    pannolini_bis_dis = get_pannolini[2]
    pannolini_bis_dis = pannolini_bis_dis.split(',')
  except:
    pannolini_bis_dis = []
  ...
  # METALLO
  if day in metallo or ((day in metallo_bis) and bool_bis) or ((day in metallo_bis_dis) and not bool_bis):
    hass.states.set('raccolta_hp.metallo', 'on', attributi_sensor)
    domani_ritirano = domani_ritirano + ["metallo"]
  else:
    hass.states.set('raccolta_hp.metallo', 'off', attributi_sensor)
  ...
  # PANNOLINI
  if day in pannolini or ((day in pannolini_bis) and bool_bis) or ((day in pannolini_bis_dis) and not bool_bis):
    hass.states.set('raccolta_hp.pannolini', 'on', attributi_sensor)
    domani_ritirano = domani_ritirano + ["pannolini"]
  else:
    hass.states.set('raccolta_hp.pannolini', 'off', attributi_sensor)    

if get_day == 'on':
  hass.states.set('input_boolean.giorno_notifica_hp', 'on', {'friendly_name': 'Raccolta Oggi', 'icon': 'mdi:update', 'Package:': 'Raccolta Differenziata', 'Creato da:': 'Mattia(xxKira) e Fabio Bigmoby Mauro', 'Creato per:': 'HassioHelp'})
  set_rifiuti(hass, oggi_s, oggi_n, prima_settimana, get_vetro, get_verde, get_umido, get_plastica, get_secco, get_metallo, get_carta, get_pannolini)
else:
  hass.states.set('input_boolean.giorno_notifica_hp', 'off', {'friendly_name': 'Raccolta domani', 'icon': 'mdi:update', 'Package:': 'Raccolta Differenziata', 'Creato da:': 'Mattia(xxKira) e Fabio Bigmoby Mauro', 'Creato per:': 'HassioHelp'})
  set_rifiuti(hass, domani_s, domani_n, prima_settimana, get_vetro, get_verde, get_umido, get_plastica, get_secco, get_metallo, get_carta, get_pannolini)
